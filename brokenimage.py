...
def audit_page(driver, url):
    ...
    for img in images:
        try:
            ...
            if alt == "":
                issues.append((url, img_url or "<empty src>", "Missing ALT text"))

        except Exception as e:
            issues.append((url, "<error obtaining src>", str(e)))
...
def crawl_site():
    ...
    while queue and len(visited) < MAX_PAGES:
        ...
        all_issues.extend(audit_page(driver, url))

        # Collect internal links
        try:
            links = driver.find_elements(By.TAG_NAME, "a")
            for link in links:
                href = normalize(link.get_attribute("href"), driver.current_url)
                if href and urlparse(href).netloc == DOMAIN and href not in visited:
                    queue.append(href)
        except:
            pass
    ...
    print("\n===== AUDIT COMPLETE =====")
    print(f"Found {len(all_issues)} images missing ALT text.")
